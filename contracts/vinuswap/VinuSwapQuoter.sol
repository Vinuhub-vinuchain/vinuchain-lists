...
import './libraries/VinuSwapPoolTicksCounter.sol';

/// @title Provides quotes for swaps
/// @notice Allows getting the expected amount out or amount in for a given swap without executing the swap
/// @dev These functions are not gas efficient and should _not_ be called on chain. Instead, optimistically execute
/// the swap and check the amounts in the callback.
contract VinuSwapQuoter is IQuoterV2, IUniswapV3SwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using VinuSwapPoolTicksCounter for IVinuSwapPool;
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IVinuSwapPool) {
        return IVinuSwapPool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function uniswapV3SwapCallback(
        ...
        IVinuSwapPool pool = getPool(tokenIn, tokenOut, fee);
        ...
        )
    {
        ...
        IVinuSwapPool pool = getPool(params.tokenIn, params.tokenOut, params.fee);
